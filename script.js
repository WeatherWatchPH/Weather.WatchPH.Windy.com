document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleAddLoan(e) {
        ...
        const loanTerm = parseInt(document.getElementById('loanTerm').value); // in weeks
        const startDate = document.getElementById('startDate').value;

        // Weekly payment calculation: One-time interest
        const weeklyPayment = calculateWeeklyPayment(loanAmount, interestRate, loanTerm);

        const paymentSchedule = generatePaymentSchedule(loanAmount, interestRate, loanTerm, startDate, weeklyPayment);

        const newLoan = {
            id: Date.now().toString(),
            name: loanName,
            originalAmount: loanAmount,
            remainingBalance: loanAmount + (loanAmount * interestRate / 100), // principal + one-time interest
            interestRate: interestRate,
            monthlyPayment: weeklyPayment, // still called monthlyPayment for table
            term: loanTerm,
            startDate: startDate,
            status: 'active',
            payments: [],
    ...
    function calculateWeeklyPayment(amount, rate, termWeeks) {
        // One-time interest added to principal
        const totalAmount = amount + (amount * rate / 100);
        return parseFloat((totalAmount / termWeeks).toFixed(2));
    }

    function generatePaymentSchedule(amount, rate, termWeeks, startDate, weeklyPayment) {
        const schedule = [];
        let balance = amount + (amount * rate / 100);
        const start = new Date(startDate);

        for (let i = 1; i <= termWeeks; i++) {
            const principal = weeklyPayment;
            const interest = 0; // interest already included in total
            balance -= principal;
    ...
    function renderLoanTable() {
        ...
        // Re-attach action buttons event listeners
    ...
    function handleMakePayment(e) {
        ...
        loan.payments.push({ amount: paymentAmount, date: paymentDate, note: paymentNote });
